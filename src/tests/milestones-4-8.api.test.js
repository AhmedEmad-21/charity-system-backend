...
test('milestone 4 - mapping API: success, validation, role', async (t) => {
  ...
  const staffRegistration = await registerUser({ role: 'Staff', email: 'map-staff@example.com' });
  const staffToken = staffRegistration.payload.data.accessToken;

  const donorRegistration = await registerUser({ role: 'Donor', email: 'map-donor@example.com' });
...
test('milestone 5 - priority APIs: get user, ranked, recalculate, missing mapping edge', async (t) => {
  ...
  const staffRegistration = await registerUser({ role: 'Staff', email: 'priority-staff@example.com' });
  const staffToken = staffRegistration.payload.data.accessToken;

  const recipientA = await registerUser({ role: 'Recipient', email: 'priority-a@example.com' });
  const recipientAId = recipientA.payload.data.user._id;

  const recipientB = await registerUser({ role: 'Recipient', email: 'priority-b@example.com' });
  ...
  const getSuccess = await jsonRequest(`/api/priority/${recipientAId}`, { token: staffToken });
  ...
  const getNotFound = await jsonRequest(`/api/priority/${notFoundId}`, { token: staffToken });
  assert.equal(getNotFound.response.status, 404);

  const ranked = await jsonRequest('/api/priority/ranked', { token: staffToken });
  ...
  assert.ok(ranked.payload.data[0].finalScore >= ranked.payload.data[1].finalScore);

  const recipientC = await registerUser({ role: 'Recipient', email: 'priority-c@example.com' });
  ...
  const recalculated = await jsonRequest(`/api/priority/recalculate/${recipientCId}`, {
    method: 'POST',
    token: staffToken,
  });
...
test('milestone 6 - donation APIs: create, validation-like required fields, status update and invalid status', async (t) => {
  ...
  const donorRegistration = await registerUser({ role: 'Donor', email: 'donor-m6@example.com' });
  const donorToken = donorRegistration.payload.data.accessToken;

  const staffRegistration = await registerUser({ role: 'Staff', email: 'staff-m6@example.com' });
  const staffToken = staffRegistration.payload.data.accessToken;
  ...
  const statusUpdated = await jsonRequest(`/api/donations/${created.payload.data._id}/status`, {
    method: 'PUT',
    token: staffToken,
    body: {
      status: 'pickedUp',
  ...
  const invalidStatus = await jsonRequest(`/api/donations/${created.payload.data._id}/status`, {
    method: 'PUT',
    token: staffToken,
    body: {
...
test('milestone 7 - inventory APIs: list, low-stock, and negative quantity error', async (t) => {
  ...
  const staffRegistration = await registerUser({ role: 'Staff', email: 'staff-m7@example.com' });
  const staffToken = staffRegistration.payload.data.accessToken;
  ...
  const inventoryList = await jsonRequest('/api/inventory', { token: staffToken });
  ...
  const lowStock = await jsonRequest('/api/inventory/low-stock', { token: staffToken });
...
test('milestone 8 - inventory movement APIs: success and quantity exceeds available', async (t) => {
  ...
  const staffRegistration = await registerUser({ role: 'Staff', email: 'staff-m8@example.com' });
  const staffToken = staffRegistration.payload.data.accessToken;
  const staffId = staffRegistration.payload.data.user._id;
