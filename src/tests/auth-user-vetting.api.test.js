...
const loginUser = async (email, password) => {
	return jsonRequest('/api/auth/login', { method: 'POST', body: { email, password } });
};
...
test('auth register success and validation/business cases', async (t) => {
	...
	const stored = await User.findOne({ email: success.payload.data.user.email }).lean();
	assert.ok(stored);
...
test('auth login and me cases', async (t) => {
	...
	const registration = await registerUser({ email: 'login@example.com', passwordHash: 'StrongPassword123' });
	assert.equal(registration.response.status, 201);

	const loginSuccess = await loginUser('login@example.com', 'StrongPassword123');
	...
	const wrongPassword = await loginUser('login@example.com', 'WrongPassword123');
	assert.equal(wrongPassword.response.status, 401);

	const meSuccess = await jsonRequest('/api/auth/me', { token: loginSuccess.payload.data.accessToken });
	...
	const meInvalidToken = await jsonRequest('/api/auth/me', { token: 'invalid-token' });
...
test('user list auth and role cases plus self update', async (t) => {
	...
	const adminRegistration = await registerUser({ role: 'Admin', email: 'admin@example.com', passwordHash: 'StrongPassword123' });
	const adminToken = adminRegistration.payload.data.accessToken;

	const recipientRegistration = await registerUser({ role: 'Recipient', email: 'recipient@example.com', passwordHash: 'StrongPassword123' });
	...
	const listForbidden = await jsonRequest('/api/users', { token: recipientToken });
	...
	const otherUserRegistration = await registerUser({ role: 'Staff', email: 'other@example.com', passwordHash: 'StrongPassword123' });
...
test('vetting request approve and reject cases', async (t) => {
	...
	const recipientRegistration = await registerUser({ role: 'Recipient', email: 'vet-recipient@example.com', passwordHash: 'StrongPassword123' });
	const recipientToken = recipientRegistration.payload.data.accessToken;
	const recipientId = recipientRegistration.payload.data.user._id;

	const donorRegistration = await registerUser({ role: 'Donor', email: 'vet-donor@example.com', passwordHash: 'StrongPassword123' });
	const donorToken = donorRegistration.payload.data.accessToken;

	const staffRegistration = await registerUser({ role: 'Staff', email: 'vet-staff@example.com', passwordHash: 'StrongPassword123' });
	const staffToken = staffRegistration.payload.data.accessToken;

	const validationRecipientRegistration = await registerUser({ role: 'Recipient', email: 'vet-validation@example.com', passwordHash: 'StrongPassword123' });
	const validationRecipientToken = validationRecipientRegistration.payload.data.accessToken;
	...
	const storedRequest = await VettingRequest.findOne({ recipientUserID: recipientId }).lean();
	...
	const approveSuccess = await jsonRequest(`/api/vetting/approve/${storedRequest._id}`, {
		method: 'PUT',
		token: staffToken,
	});
	...
	const points = await RecipientPoints.findOne({ recipientUserID: recipientId }).lean();
	assert.ok(points);

	const priority = await RecipientPriority.findOne({ recipientUserID: recipientId }).lean();
	assert.ok(priority);

	const approveTwice = await jsonRequest(`/api/vetting/approve/${storedRequest._id}`, {
		method: 'PUT',
		token: staffToken,
	});
	...
	const rejectSuccess = await jsonRequest(`/api/vetting/reject/${rejectRequest.payload.data._id}`, {
		method: 'PUT',
		token: staffToken,
	...
	const nonStaffApprove = await jsonRequest(`/api/vetting/approve/${rejectRequest.payload.data._id}`, {
		method: 'PUT',
		token: recipientToken,
	});
