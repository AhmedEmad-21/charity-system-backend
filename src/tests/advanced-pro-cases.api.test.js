...
test("advanced auth: spaces in email, case-insensitive email, special chars, token security", async (t) => {
  ...
  const manipulatedTokenRes = await jsonRequest("/api/auth/me", {
  ...
  const expiredToken = jwt.sign(
    {
      id: registration.payload.data.user._id,
      role: "Donor",
      tokenType: "access",
    },
  ...
  const expiredTokenRes = await jsonRequest("/api/auth/me", {
    token: expiredToken,
  });
...
test("advanced user: self email update, role-protected update, concurrent updates, linked delete safety", async (t) => {
  ...
  const forbiddenRoleUpdate = await jsonRequest(
    `/api/users/${userB.payload.data.user._id}`,
    {
      method: "PUT",
      token: userAToken,
      body: {
  ...
  assert.ok(
    concurrentUpdateResults.every((entry) => entry.response.status === 200),
  );
  ...
  const linkedRequestStillExists = await RecipientRequest.findById(
    reqRes.payload.data._id,
  ).lean();
...
test("advanced vetting: approve race, reject-after-approve, approve-after-reject, income/docs validation", async (t) => {
  ...
  const staff1 = await registerUser({
    role: "Staff",
    email: uniqueEmail("staff1"),
  });
  const staff2 = await registerUser({
    role: "Staff",
    email: uniqueEmail("staff2"),
  });
  const recipient = await registerUser({
    role: "Recipient",
    email: uniqueEmail("vet-rec"),
  });
  ...
  const approveRace = await Promise.all([
    jsonRequest(`/api/vetting/approve/${vettingId}`, {
      method: "PUT",
  ...
  const raceSuccess = approveRace.filter(
    (entry) => entry.response.status === 200,
  ).length;
  const raceRejected = approveRace.filter(
    (entry) => entry.response.status >= 400,
  ).length;
  assert.equal(raceSuccess, 1);
  assert.equal(raceRejected, 1);

  const rejectAfterApprove = await jsonRequest(
    `/api/vetting/reject/${vettingId}`,
    {
      method: "PUT",
      token: staff1.payload.data.accessToken,
  ...
  const recipient2 = await registerUser({
    role: "Recipient",
    email: uniqueEmail("vet-rec-2"),
  });
  ...
  const rejectFirst = await jsonRequest(
    `/api/vetting/reject/${req2.payload.data._id}`,
    {
      method: "PUT",
      token: staff1.payload.data.accessToken,
      body: { notes: "rejected" },
  ...
  const approveAfterReject = await jsonRequest(
    `/api/vetting/approve/${req2.payload.data._id}`,
    {
      method: "PUT",
      token: staff2.payload.data.accessToken,
    },
  ...
  const validationRecipient = await registerUser({
    role: "Recipient",
    email: uniqueEmail("vet-validation"),
  });
  ...
  const tieResolution = getConflictResolutionTier(
    {
      ...
      priority: 10,
      requestDate: new Date("2026-01-01T10:00:00.000Z"),
    },
    {
      _id: new mongoose.Types.ObjectId(),
      priority: 10,
      requestDate: new Date("2026-01-01T10:05:00.000Z"),
    },
  );
...
test("advanced donations and transactions: status flow, double-click idempotency, amount edge cases", async (t) => {
  ...
  const donor = await registerUser({
    role: "Donor",
    email: uniqueEmail("donor-adv"),
  });
  const donorToken = donor.payload.data.accessToken;
  const staff = await registerUser({
    role: "Staff",
    email: uniqueEmail("don-staff"),
  });
  const staffToken = staff.payload.data.accessToken;
  ...
  const jumpStatus = await jsonRequest(
  ...
  const cancelStatus = await jsonRequest(
    `/api/donations/${donation.payload.data._id}/status`,
    {
      method: "PUT",
      token: staffToken,
  ...
  const txDocs = await Transaction.find({
    donorID: donor.payload.data.user._id,
  }).lean();
...
test("advanced inventory and movement: race updates, edges, and consistency with movements", async (t) => {
  ...
  assert.ok(
    updateRace.every(
      (entry) => entry.response.status === 200 || entry.response.status === 500,
    ),
  );
  ...
  const missingInventoryMovement = await jsonRequest(
    "/api/inventory-movements/move",
    {
      method: "POST",
      token,
      body: {
  ...
  const missingStaffMovement = await jsonRequest(
    "/api/inventory-movements/move",
    {
      method: "POST",
      token,
...
test("advanced recipient-request and review: concurrency, edge, abuse, and double-approval safety", async (t) => {
  ...
  const staff = await registerUser({
    role: "Staff",
    email: uniqueEmail("req-staff"),
  });
  const staffToken = staff.payload.data.accessToken;
  const staffId = staff.payload.data.user._id;

  const recipient1 = await registerUser({
    role: "Recipient",
    email: uniqueEmail("req-r1"),
  });
  const recipient2 = await registerUser({
    role: "Recipient",
    email: uniqueEmail("req-r2"),
  });
  ...
  const scarceInv = await createInventoryDoc({
    quantity: 1,
    itemPointsCost: 3,
    monthlyLimit: 2,
  });
  ...
  const approvedCount = approveRace.filter(
    (entry) => entry.response.status === 200,
  ).length;
  const rejectedCount = approveRace.filter(
    (entry) => entry.response.status >= 400,
  ).length;
  ...
  const noItemsReview = await jsonRequest(
    `/api/recipient/review/${noItemsRequest.payload.data._id}`,
    {
      method: "PUT",
      token: staffToken,
      body: { status: "approved" },
  ...
  const lowPointsRecipient = await registerUser({
    role: "Recipient",
    email: uniqueEmail("req-low"),
  });
  await approveRecipientAndSeedPoints(
    lowPointsRecipient.payload.data.user._id,
    0,
  );
  ...
  const abuseUser = await registerUser({
    role: "Recipient",
    email: uniqueEmail("abuse"),
  });
  ...
  const abuseSuccess = abuseResults.filter(
    (entry) => entry.response.status === 201,
  ).length;
  assert.equal(abuseSuccess, 10);

  const winnerReq = await RecipientRequest.findOne({
    status: "approved",
  }).lean();
  if (winnerReq) {
    const secondApprove = await jsonRequest(
      `/api/recipient/review/${winnerReq._id}`,
      {
        method: "PUT",
        token: staffToken,
        body: { status: "approved" },
    ...
    assert.ok(
      secondApprove.response.status >= 400 ||
        secondApprove.response.status === 200,
    );
  }
...
test("advanced points, messages, jobs, global load and security", async (t) => {
  ...
  const admin = await registerUser({
    role: "Admin",
    email: uniqueEmail("jobs-admin"),
  });
  const staff = await registerUser({
    role: "Staff",
    email: uniqueEmail("jobs-staff"),
  });
  const recipient = await registerUser({
    role: "Recipient",
    email: uniqueEmail("msg-recipient"),
  });
  const otherRecipient = await registerUser({
    role: "Recipient",
    email: uniqueEmail("msg-other"),
  });
  ...
  const roleBypassAttempt = await jsonRequest(
    "/api/configs/system/run-monthly-reset",
    {
      method: "POST",
      token: recipient.payload.data.accessToken,
    },
  ...
  const idGuessing = await jsonRequest(
    `/api/messages/read/${new mongoose.Types.ObjectId()}`,
    {
      method: "PUT",
      token: recipient.payload.data.accessToken,
    },
  ...
  assert.ok(
    idGuessing.response.status === 404 || idGuessing.response.status === 403,
  );
  ...
  const beforePoints = await RecipientPoints.findOne({
    recipientUserID: recipient.payload.data.user._id,
  }).lean();
  ...
  assert.equal(
    highLoad.filter((entry) => entry.response.status === 200).length,
    100,
  );
  ...
  assert.ok(
    largePayload.response.status === 201 ||
      largePayload.response.status === 400,
  );
...
test("advanced full-flow: donor -> inventory -> request -> priority -> approve -> points/inventory/message consistency", async (t) => {
  ...
  const donor = await registerUser({
    role: "Donor",
    email: uniqueEmail("full-donor"),
  });
  const staff = await registerUser({
    role: "Staff",
    email: uniqueEmail("full-staff"),
  });
  const recipient = await registerUser({
    role: "Recipient",
    email: uniqueEmail("full-recipient"),
  });
  ...
  const vettingApprove = await jsonRequest(
    `/api/vetting/approve/${vettingRequest.payload.data._id}`,
    {
      method: "PUT",
  ...
  const priority = await jsonRequest(
    `/api/priority/recalculate/${recipient.payload.data.user._id}`,
    {
      method: "POST",
      token: staff.payload.data.accessToken,
    },
  ...
  const approve = await jsonRequest(
    `/api/recipient/review/${requestCreated.payload.data._id}`,
    {
      method: "PUT",
      token: staff.payload.data.accessToken,
      body: { status: "approved" },
  ...
  const pointsAfter = await RecipientPoints.findOne({
    recipientUserID: recipient.payload.data.user._id,
  }).lean();
  ...
  const requestAfter = await RecipientRequest.findById(
    requestCreated.payload.data._id,
  ).lean();
  ...
  const pointsTx = await PointsTransaction.findOne({
    relatedRequestID: requestCreated.payload.data._id,
  }).lean();
  assert.ok(pointsTx);

  const duplicateTx = await PointsTransaction.countDocuments({
    relatedRequestID: requestCreated.payload.data._id,
  });
  ...
  const allRequestedItems = await RequestedItem.find({
    recipientRequestID: requestCreated.payload.data._id,
  }).lean();
