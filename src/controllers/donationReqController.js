...
const updateDonationStatus = async (req, res, next) => {
  try {
    ...
    // ✅ التعديل هنا: نأخذ كامل الـ body لضمان انتقال الـ quantity وأي حقول إضافية للـ Service
    const updateData = req.body;

    const before = await donationReqService.fetchDonationById(id);

    // ✅ التعديل هنا: نمرر updateData كاملة
    const updated = await donationReqService.updateStatus(id, updateData, { session: req.mongoSession || null });
    ...
    setAuditEntry(req, 'status_updated', {
      status: 'failed',
      before: null,
      after: { id: req.params.id, data: req.body },
      error,
      metadata: { actorUserID: req.user?.id || req.user?._id || req.user?.userId || null },
    });
    return next(error);
